...
class Review:
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "Review":
        username = d.get("username") or d.get("user") or "unknown"
        rating = d.get("rating")
        comment = d.get("raw_text") or d.get("comment") or ""
...
class CorpusDocument:
    def __init__(self, review: Review, processed: Optional[Dict[str, Any]] = None):
        ...
        self.text = {
            ...
            "pos_tags": [], "dependencies": [], "num_tokens": 0, "num_sentences": 0
        }
        ...
        if processed:
            ...
            self.text.update({
                "sentences": processed.get("sentences", []) or [],
                "tokens": processed.get("tokens", []) or [],
                "tokens_no_stopwords": processed.get("tokens_no_stopwords", []) or [],
                "stems": processed.get("stems", []) or [],
                "lemmas": processed.get("lemmas", []) or [],
                "pos_tags": processed.get("pos_tags", []) or [],
                "dependencies": processed.get("dependencies", []) or [],
                "num_tokens": processed.get("num_tokens", 0),
                "num_sentences": processed.get("num_sentences", 0),
            })
            
            patterns_dict = processed.get("patterns", {}) or {}
            for k in self.patterns:
                self.patterns[k] = patterns_dict.get(k, []) or []
...
class GameCorpus:
    def __init__(self, game_id: Optional[Any] = None, metadata: Optional[Dict[str, Any]] = None,
        ...
        self.documents: List[CorpusDocument] = documents if documents else []
    ...
    def from_dict(cls, d: Dict[str, Any]) -> "GameCorpus":
        gid = d.get("game_id") or (d.get("metadata", {}).get("game_info", {}).get("id"))
        try:
            gid = int(gid) if gid is not None else None
        except Exception:
            pass
...
class MongoCorpusStorage:
    """
    Gestor de almacenamiento MongoDB para Corpus de BGG.
    """
...
class Corpus:
    """Corpus como colección de GameCorpus."""
    ...
    def from_json(cls, path: str) -> "Corpus":
        ...
        if isinstance(data, list):
            for game_obj in data:
                games.append(GameCorpus.from_dict(game_obj))
            return cls(games=games)

        if isinstance(data, dict) and "reviews" in data and "metadata" in data:
            legacy_meta = data.get("metadata", {})
            games_metadata = legacy_meta.get("games") if isinstance(legacy_meta.get("games"), dict) else {}

            for gid_str, gmeta in (games_metadata or {}).items():
                try:
                    gid = int(gid_str)
                except Exception:
                    gid = gid_str
                games.append(GameCorpus(game_id=gid, metadata=gmeta, documents=[]))

            for r in data.get("reviews", []):
                gid = r.get("game_id") or None
                rev = Review.from_dict({**r, "game_id": gid})
                processed = {}
                if "text" in r or "patterns" in r or "clean_text" in r:
                    processed["clean_text"] = r.get("clean_text")
                ...
                target = next((g for g in games if g.game_id == gid), None)
                if not target:
                    target = GameCorpus(game_id=gid, metadata={})
                    games.append(target)
                target.add_document(doc)
    ...
    def words(self, game_ids: Optional[List[Any]] = None, categories: Optional[List[str]] = None,
        ...
        docs = self._select(game_ids=game_ids, categories=categories, labels=labels)
        return [tok for d in docs for tok in d.text.get("tokens", [])]

    def sents(self, i: Optional[int] = None):
        if i is None:
            return [doc.text.get("sentences", []) for doc in self.documents]
        return self.documents[i].text.get("sentences", [])
    ...
    # -------------------------
    # Estadísticas / utilidades (operan sobre la vista plana o sobre metadata por juego)
    # -------------------------
    ...
    def lexical_dispersion_plot(self, words: List[str], game_ids: Optional[List[Any]] = None, categories: Optional[List[str]] = None):
        ...
        all_tokens = []
        for doc in docs:
            all_tokens.extend(doc.text.get("tokens", []))
        points = []
        for word in words:
    ...
    def frequency_distribution(self, game_ids: Optional[List[Any]] = None, categories: Optional[List[str]] = None) -> Counter:
        docs = self._select(game_ids=game_ids, categories=categories)
        all_tokens = [tok.lower() for doc in docs for tok in doc.text.get("tokens", [])]
    ...
    def word_length_distribution(self, game_ids: Optional[List[Any]] = None, categories: Optional[List[str]] = None):
        docs = self._select(game_ids=game_ids, categories=categories)
        lengths = [len(tok) for doc in docs for tok in doc.text.get("tokens", [])]
    ...
    def ngrams(self, n: int = 2, game_ids: Optional[List[Any]] = None, categories: Optional[List[str]] = None):
        docs = self._select(game_ids=game_ids, categories=categories)
        tokens = [tok.lower() for doc in docs for tok in doc.text.get("tokens", [])]
